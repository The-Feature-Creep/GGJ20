...
export default class Car extends THREE.Object3D{
	constructor(){
		...
		//  Wheels
		var geometry = new THREE.CylinderBufferGeometry( 6, 6, 4, 32 );
	
		let lb_wheel = new THREE.Mesh( geometry, wheel_material );
		lb_wheel.rotation.z += Math.PI/2;
		this.add(lb_wheel);

		let rb_wheel = new THREE.Mesh( geometry, wheel_material );
		rb_wheel.rotation.z += Math.PI/2;
		rb_wheel.position.x = body_width;
		this.add(rb_wheel);

		let lf_wheel = new THREE.Mesh( geometry, wheel_material );
		lf_wheel.rotation.z += Math.PI/2;
		lf_wheel.position.z = body_length;
		this.add(lf_wheel);

		let rf_wheel = new THREE.Mesh( geometry, wheel_material );
